...
import com.library.auth.dto.StudentDetailDTO;
import com.library.auth.dto.StudentSummaryDTO;
import com.library.auth.entity.UserEntity;
import com.library.auth.exception.ResourceNotFoundException;
...
public class AdminStudentService {
    ...
    public StudentDetailDTO getStudentDetail(Long id) {
        ...
        List<BookingHistoryDTO> bookings = fetchBookingHistory(user.getStudentId());
        List<PaymentHistorySummaryDTO> payments = fetchPaymentHistory(user.getStudentId());
    ...
    private List<BookingHistoryDTO> fetchBookingHistory(String studentId) {
        try {
            URI uri = Objects.requireNonNull(URI.create(
                    String.format("%s/api/bookings?studentId=%s&page=0&size=50", seatServiceUrl, studentId)));
            ...
            ResponseEntity<Map<String, Object>> response = restTemplate.exchange(uri, method, entity,
            ...
            // Fallback to empty list when remote service is unavailable
        }
    ...
    private List<PaymentHistorySummaryDTO> fetchPaymentHistory(String studentId) {
        try {
            URI uri = Objects.requireNonNull(URI.create(
                    String.format("%s/api/payments?studentId=%s", paymentServiceUrl, studentId)));
            HttpHeaders headers = buildAuthHeaders();
            HttpMethod method = Objects.requireNonNull(HttpMethod.GET);
            HttpEntity<Void> entity = new HttpEntity<>(Objects.requireNonNull(headers));
            ResponseEntity<Map<String, Object>> response = restTemplate.exchange(uri, method, entity,
