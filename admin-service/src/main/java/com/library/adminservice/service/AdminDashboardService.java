...
public class AdminDashboardService {
    ...
    public AdminDashboardDTO getDashboard() {
        ...
        int totalSeats = occupancy.stream()
                .mapToInt(e -> toInt(e.get("available")) + toInt(e.get("occupied")) + toInt(e.get("maintenance")))
                .sum();
    ...
    private int fetchActiveBookingsToday() {
        try {
            URI uri = URI.create(adminUrlProperties.getSeatServiceUrl() + "/api/bookings/today");
    ...
    private List<Map<String, Object>> fetchSeatOccupancy() {
        try {
            URI uri = URI.create(adminUrlProperties.getSeatServiceUrl() + "/api/admin/seats/occupancy");
            ResponseEntity<List<Map<String, Object>>> response = restTemplate.exchange(uri, HttpMethod.GET,
                    new HttpEntity<>(buildAuthHeaders()),
    ...
    private BigDecimal fetchMonthlyRevenue() {
        try {
            ...
            URI uri = URI.create(adminUrlProperties.getPaymentServiceUrl()
    ...
    private List<BookingHistoryDTO> fetchRecentBookings() {
        try {
            URI uri = URI.create(adminUrlProperties.getSeatServiceUrl()
